...
public class ReviewServiceImpl implements ReviewService {
    ...
    public boolean create(Review review) throws ValidationException, DatabaseInteractionException {
        boolean wasCreated;
        ValidationChain<Review> chain = ValidationChainFactory.INSTANCE.createValidationChain(review);
        List<String> validationErrors = chain.getValidationReport(review, ValidationType.CREATE_OBJECT);

        if (validationErrors.size() == 0) {
            wasCreated = ReviewDaoImpl.getInstance()
                    .create(review, ConnectionPool.INSTANCE.getAvailableConnection());
    ...
    public List<Review> findAllForConcreteUser(Long id) throws DatabaseInteractionException {
        return ReviewDaoImpl.getInstance()
                .findAllForConcreteUser(id, ConnectionPool.INSTANCE.getAvailableConnection());
    }

    @Override
    public Review updateReviewMarks(Review review) throws DatabaseInteractionException {
        return ReviewDaoImpl.getInstance()
                .updateReviewMarks(review, ConnectionPool.INSTANCE.getAvailableConnection());
    }

    @Override
    public List<Review> findAllForConcreteUserInReview(Long id) throws DatabaseInteractionException {
        return ReviewDaoImpl.getInstance()
                .findAllForConcreteUserInReview(id, ConnectionPool.INSTANCE.getAvailableConnection());
    }

    @Override
    public List<Review> findAllForConcreteProductInReview(Long id) throws DatabaseInteractionException {
        return ReviewDaoImpl.getInstance()
                .findAllForConcreteProductInReview(id, ConnectionPool.INSTANCE.getAvailableConnection());
    }

    @Override
    public boolean transferInHistoryTable(List<Review> reviews) throws DatabaseInteractionException {
        return ReviewDaoImpl.getInstance()
                .transferInHistoryTable(reviews, ConnectionPool.INSTANCE.getAvailableConnection());
    }

    @Override
    public List<Integer> getAllProductMarks(Long id) throws DatabaseInteractionException {
        return ReviewDaoImpl.getInstance()
                .getAllProductMarks(id, ConnectionPool.INSTANCE.getAvailableConnection());
    }
